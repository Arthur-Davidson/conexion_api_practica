    ...
    static func descargar_informacion<Tipo: Codable>(desde: String) async ->Tipo? {
        do {
            ...
            guard let respuesta = respuesta as? HTTPURLResponse else { throw
                ErroresAPI.respuesta_erronea }
            guard respuesta.statusCode >= 200 && respuesta.statusCode < 300 else { throw
                ErroresAPI.codigo_respuesta_erronea }
            guard let datos_descodificados = try? JSONDecoder().decode(Tipo.self, from: datos) else { throw
                ErroresAPI.falla_en_la_codificacion }
